...
public class midChancellorTests {
    ...
    private final String networkName = "networks/mid/MID-Chancellor.pgmx";
    ...
    public void veOptimalIntervention() {
        ...
        try {
            ...
            Assert.assertTrue(optimalIntervention.getRootVariable().equals(therapyType));
            Assert.assertTrue(veOptimalIntervention.getOptimalIntervention().getBranches().size() == 1);

            TreeADDBranch branchCombinationTherapy = veOptimalIntervention.getOptimalIntervention().getBranches().get(0);
    ...
    public void stateVETemporalEvolutionTests(){
        ...
        try {
            VETemporalEvolution veTemporalEvolution = new VETemporalEvolution(probNet, stateVariable, preResolutionEvidence, null);
            HashMap<Variable,TablePotential> posteriorValues = veTemporalEvolution.getPosteriorValues();
            ...
            valuesToCheck = new double[]{0.014226902493050322, 0.08022683278358073, 0.16900517143388435, 0.7365410932894846};
            Assert.assertArrayEquals(valuesToCheck,posteriorValues.get(variableToCheck).getValues(),deltaEquals);

            // Check State [3]
            variableToCheck = veTemporalEvolution.getExpandedNetwork().getVariable("State",3);
            valuesToCheck = new double[]{0.04350829929448966, 0.17823853605004344, 0.24687317178302465, 0.5313799928724422};
            Assert.assertArrayEquals(valuesToCheck,posteriorValues.get(variableToCheck).getValues(),deltaEquals);

            // Check State [4]
            variableToCheck = veTemporalEvolution.getExpandedNetwork().getVariable("State",4);
            valuesToCheck = new double[]{0.09619570266402257, 0.26972849455807596, 0.2507099601881366, 0.38336584258976464};
            Assert.assertArrayEquals(valuesToCheck,posteriorValues.get(variableToCheck).getValues(),deltaEquals);

            // Check State [5]
            variableToCheck = veTemporalEvolution.getExpandedNetwork().getVariable("State",5);
            valuesToCheck = new double[]{0.17033714773860123, 0.3300188414850077, 0.22306346345041067, 0.27658054732598036};
            Assert.assertArrayEquals(valuesToCheck,posteriorValues.get(variableToCheck).getValues(),deltaEquals);

            // Check State [10]
            variableToCheck = veTemporalEvolution.getExpandedNetwork().getVariable("State",10);
            valuesToCheck = new double[]{0.6088617435803294, 0.2709176367277771, 0.06616227856975207, 0.05405834112214166};
            Assert.assertArrayEquals(valuesToCheck,posteriorValues.get(variableToCheck).getValues(),deltaEquals);

            // Check State [15]
            variableToCheck = veTemporalEvolution.getExpandedNetwork().getVariable("State",15);
            valuesToCheck = new double[]{0.8637223703879661, 0.11128536068983778, 0.014426434580920574, 0.010565834341275641};
            Assert.assertArrayEquals(valuesToCheck,posteriorValues.get(variableToCheck).getValues(),deltaEquals);

            // Check State [19]
            variableToCheck = veTemporalEvolution.getExpandedNetwork().getVariable("State",19);
            valuesToCheck = new double[]{0.9472612120920146, 0.04584774372014692, 0.00402860239965864, 0.002862441788179781};
            Assert.assertArrayEquals(valuesToCheck,posteriorValues.get(variableToCheck).getValues(),deltaEquals);
